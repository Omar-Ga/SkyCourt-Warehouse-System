-- database/migrations/002_two_operator_overhaul.sql
-- Establishes authenticated workflows, order lifecycle, idempotency operations, and audit linkages.
...
-- Operations Table (Idempotency and Atomic Execution)
...
-- Add additive audit columns to Movement Logs
ALTER TABLE movement_logs ADD COLUMN user_id INTEGER REFERENCES users(id) ON DELETE RESTRICT;
ALTER TABLE movement_logs ADD COLUMN actor_name TEXT;
ALTER TABLE movement_logs ADD COLUMN operation_key TEXT;
ALTER TABLE movement_logs ADD COLUMN po_line_id INTEGER REFERENCES purchase_order_items(id) ON DELETE RESTRICT;
ALTER TABLE movement_logs ADD COLUMN leave_line_id INTEGER REFERENCES leave_order_items(id) ON DELETE RESTRICT;
ALTER TABLE movement_logs ADD COLUMN return_event_id INTEGER REFERENCES return_events(id) ON DELETE RESTRICT;
