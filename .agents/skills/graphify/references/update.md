...
Read for freshness assessment or an index refresh. Numbered pipeline steps below refer to [build.md](build.md); load those steps only when performing the manual update.

## Freshness before retrieval

Check `.graphify_root` against the current checkout and use `manifest.json` to compare relevant indexed files with current content. Git status identifies working changes but cannot identify all changes since indexing. Include added/untracked files with source search and account for deleted/renamed files. If index identity or coverage is unknown, verify source and treat graph results as hints.

When a broader check is justified, the installed Python API `detect_incremental(Path(root), manifest_path=..., kind="ast")` compares structural coverage; `kind="semantic"` checks semantic coverage. Print counts and relevant paths only. Do not stamp the manifest during a check. Semantic freshness is distinct from AST freshness.

Prefer the installed `graphify update <root>` for a supported structural refresh; inspect its capabilities before choosing flags. It does not replace semantic extraction. Preserve existing direction and corpus scope. If a CLI cannot preserve these, use a compatible manual update or proceed with source verification. Do not rebuild after each query or edit.

The manual procedure below is version-sensitive. Keep a backup before merge, use only this run's extraction, prune deleted files, and stamp successfully extracted sources only. A failed update must not make the old graph appear fresh.
...
```bash
# Always replace stale intermediate extraction on a deletions-only run.
...
# the existing graph’s directed setting for updates. Without it a --directed --update silently rebuilds undirected and collapses
