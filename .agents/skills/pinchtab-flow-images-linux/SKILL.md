---
name: pinchtab-flow-images-linux
...
- PinchTab server must be running (use `pinchtab server -b`)
- A headed Brave instance on the `default_profile` named profile must be running
  ...
  (see `pinchtab-account-setup-linux` skill for profile management)
...
Downloaded files land in `~/Downloads`. Move them into the active workspace root:

```bash
ls -t ~/Downloads | head -n <count> | xargs -I {} cp "$HOME/Downloads/{}" ./public/images/
```
