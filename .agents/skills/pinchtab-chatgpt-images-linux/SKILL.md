---
name: pinchtab-chatgpt-images-linux
disable-model-invocation: true
description: >
  STRICTLY OFF BY DEFAULT. Only invoke this skill when the user explicitly requests PinchTab or pinchtab by name in their prompt. Do NOT activate implicitly for general browsing, web searching, or scraping. Automate image generation on ChatGPT (chatgpt.com) on Linux using PinchTab browser automation.
  Covers multi-account rate limit rotation across Linux profiles (profile_1, profile_2, profile_3, profile_4),
...
# ChatGPT Image Generation — PinchTab Automation Skill (Linux)

## Overview

This skill teaches agents how to drive **ChatGPT's native image generator** (`gptimage2` / DALL-E 3 / GPT-4o Image Gen) end-to-end via the PinchTab CLI on Linux.
...
1. **PinchTab Server**: Must be running in background:
   ```bash
   pinchtab server -b
   ```
